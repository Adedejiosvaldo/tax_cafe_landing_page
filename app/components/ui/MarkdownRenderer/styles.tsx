...
const DeletedText = ({ className, ...props }: DeletedTextProps) => (
  <del
    className={cn(className, 'text-text-light-body line-through', PARAGRAPH_SIZES.body)}
    {...filterProps(props)}
  />
)

const HorizontalRule = ({ className, ...props }: HorizontalRuleProps) => (
  <hr className={cn(className, 'border-border-light mx-auto w-48 border-b')} {...filterProps(props)} />
)

const InlineCode: FC<PreparedTextProps> = ({ children }) => {
  return (
    <code className="bg-background-light relative whitespace-pre-wrap rounded-sm p-1">
      {children}
    </code>
  )
}
...
const Img = ({ src, alt }: ImgProps) => {
  ...
  return (
    <div className="w-full max-w-xl">
      {error ? (
        <div className="bg-background-light text-text-light-body flex h-40 flex-col items-center justify-center gap-2 rounded-md">
          <Paragraph className="text-gray-900">Image unavailable</Paragraph>
          <Link
...
const Table = ({ className, ...props }: TableProps) => (
  <div className="border-border-light w-full max-w-[560px] overflow-hidden rounded-md border">
...
const TableHead = ({ className, ...props }: TableHeaderProps) => (
  <thead
    className={cn(
      className,
      'border-border-light rounded-md border-b bg-transparent p-2 text-left text-sm font-[600]'
    )}
    {...filterProps(props)}
  />
)
...
const TableRow = ({ className, ...props }: TableRowProps) => (
  <tr className={cn(className, 'border-border-light border-b last:border-b-0')} {...filterProps(props)} />
)
